/**
 * PiAdapterLive - Effect Layer bridging PiRpcManager to ProviderAdapterShape.
 *
 * Wraps PiRpcManager in the Effect layer system so it can be composed into the
 * ProviderAdapterRegistry alongside Codex, Copilot, Kimi, and Droid.
 *
...
export interface PiAdapterLiveOptions {
  ...
  readonly makeManager?: (stateDir: string) => PiRpcManager;
}
...
function toSessionError(
  ...
  if (normalized.includes("no active pi session")) {
  ...
  if (normalized.includes("session is closed") || normalized.includes("stopping")) {
...
export const makePiAdapterLive = (options?: PiAdapterLiveOptions) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const manager =
        options?.manager ??
        options?.makeManager?.(serverConfig.stateDir) ??
        new PiRpcManager(serverConfig.stateDir);
      ...
      const streamEvents: PiAdapterShape["streamEvents"] = Stream.fromQueue(eventQueue);
