...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const [sendStartedAtByThreadId, setSendStartedAtByThreadId] = useState<Record<string, string>>(
    {},
  );
  ...
  const setThreadSendStartedAt = useCallback(
    (targetThreadId: ThreadId, startedAt: string | null) => {
      setSendStartedAtByThreadId((existing) => {
        const current = existing[targetThreadId] ?? null;
        ...
        return {
          ...existing,
          [targetThreadId]: startedAt,
        };
      });
    },
    [],
  );
  const beginThreadSendPhase = useCallback(
    (targetThreadId: ThreadId, nextPhase: Exclude<SendPhase, "idle">) => {
      setSendStartedAtByThreadId((existing) => {
        ...
        return {
          ...existing,
          [targetThreadId]: new Date().toISOString(),
        };
      });
  ...
  const resetThreadSendPhase = useCallback(
    (targetThreadId: ThreadId) => {
      setThreadSendPhase(targetThreadId, "idle");
      setThreadSendStartedAt(targetThreadId, null);
    },
    [setThreadSendPhase, setThreadSendStartedAt],
  );
  ...
  const sendStartedAt = sendStartedAtByThreadId[threadId] ?? null;
  ...
  useEffect(() => {
    if (!isWorking || !activeWorkStartedAt) return;
    setNowTick(Date.now());
  ...
  }, [activeWorkStartedAt, isWorking]);

  useEffect(() => {
    if (!activeThreadId || sendPhase === "idle") {
      return;
    }
    if (
      phase === "running" ||
      ...
      resetThreadSendPhase(activeThreadId);
    }
  }, [
  ...
  beginThreadSendPhase(
    ...
    await (async () => {
      ...
      if (baseBranchForWorktree) {
        beginThreadSendPhase(threadIdForSend, "preparing-worktree");
  ...
  beginThreadSendPhase(threadIdForSend, "sending-turn");
    ...
    if (!turnStartSucceeded) {
      resetThreadSendPhase(threadIdForSend);
    }
  };
  ...
  const onImplementPlanInNewThread = useCallback(async () => {
    ...
    beginThreadSendPhase(activeThread.id, "sending-turn");
    const finish = () => {
      setThreadSendInFlight(activeThread.id, false);
      resetThreadSendPhase(activeThread.id);
    };
